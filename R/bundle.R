...
createAppManifest <- function(appDir, appMode, contentCategory, hasParameters,
  ...
  if (length(missing_url_sources)) {
    ...
    # always the cause.
    msg <- c(msg, sprintf("Unable to determine the location for some packages. Packages must come from a package repository like CRAN or a source control system. Check that options('repos') refers to a package repository containing the needed package versions. Problematic packages include: %s", paste(missing_url_sources, collapse=",")))
  }
